namespace :big_sam do
  task :letters_from_csv, [:csv] => :environment do |t, args|
    ...
    sheet.each_with_index do |row, idx|
      next if idx == 0
      rows.push(Hash[[headers, row].transpose].symbolize_keys)
    end
    ...
    rows.each do |row|
      ...
      letter.places_written.clear
      letter.places_sent.clear
      ...
      if row[:reg_recipient]
        row[:reg_recipient].split(';').each do |recipient|
          ...
          begin
            ...
            LetterRecipient.find_or_create_by(letter: letter, entity: entity)
      ...
      if row[:first_repository]
        ...
        if row[:euro_or_am]
          repo.american = row[:euro_or_am].downcase == 'american' ? true : false
        end
        if row[:first_public]
          repo.public = row[:first_public].downcase == 'public' ? true : false
        end
        begin
      ...
      if row[:second_repository]
        ...
        if row[:second_public]
          repo.public = row[:second_public].downcase == 'public' ? true : false
        end
	begin
  ...
  def get_entity(label, type)
    entity = nil
    _label = label.downcase.strip.gsub(/[\[!@%&?"\]]/, '')
    if type == 'person'
      entity = Entity.where('LOWER(entities.label) = ?', label.downcase.strip.gsub(/[\[!@%&?"\]]/, '')).first
      if entity.nil?
        Entity.by_type('person').each do |e|
          if e.properties['alternate_names_spellings'].map{|n| n.downcase}.include? _label
            entity = e
          end
        end
      end
